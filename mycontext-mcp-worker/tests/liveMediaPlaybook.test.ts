...
const liveIt = process.env.LIVE_MEDIA_PLAYBOOK_SMOKE === "1" ? it : it.skip;

describe("live media playbook MCP smoke", () => {
  liveIt("returns the whole playbook from one TiDB record", async () => {
    const databaseUrl = await readDevVar("TIDB_DATABASE_URL");
    const server = new McpServer({ name: "live-media-smoke", version: "1.0.0" });
    ...
    try {
      ...
      expect(result.content).toEqual([{
        type: "text",
        text: expect.stringMatching(/^# メディア運営プレイブック[\s\S]+/)
      }]);
